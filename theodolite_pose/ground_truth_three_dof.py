...
class GroundTruth(Node):
    def __init__(self):
        ...
        self.create_subscription(
            TheodoliteCoordsStamped, input_topic, self.theodolite_callback, 10
        )
        self.tf_sub = self.create_subscription(TFMessage, "/tf", self.tf_callback, 10)
        self.tf_static_sub = self.create_subscription(
            TFMessage, "/tf_static", self.tf_callback, qos_profile_action_status_default
        )
    ...
    def theodolite_callback(self, msg):
        ...
        if msg.status == 0:
            ...
            position = self.theodo_to_cartesian(
                msg.distance, msg.azimuth, msg.elevation
            )
            ...
            if self.calibration_computed:
                ...
                self.prism_id == 2 and self.nb_poses >= self.min_measurements
            ):  # and self.euclidian_distance(position, self.prism_positions[0]) < 0.1:
    ...
    def tf_callback(self, tf_msg):
        for tf in tf_msg.transforms:
            if (
                tf.child_frame_id == "prism1"
                and tf.header.frame_id == "base_link"
                and self.Q1 is None
            ):
                ...
                self.get_logger().info(f"Prism1 -> base_link: {self.Q1}")
            elif (
                tf.child_frame_id == "prism2"
                and tf.header.frame_id == "base_link"
                and self.Q2 is None
            ):
                self.Q2 = np.array(
                ...
                self.get_logger().info(f"Prism2 -> base_link: {self.Q2}")
            elif (
                tf.child_frame_id == "prism3"
                and tf.header.frame_id == "base_link"
                and self.Q3 is None
            ):
                self.Q3 = np.array(
                ...
                self.get_logger().info(f"Prism3 -> base_link: {self.Q3}\n")
            elif (
                tf.child_frame_id == "base_link"
                and tf.header.frame_id == "odom"
                and self.T_odom_to_base_link is None
            ):
                self.T_odom_to_base_link = self.tfTransform_to_matrix(tf.transform)
                self.get_logger().info(f"Odom -> base_link transformation acquired.\n")
            elif (
                tf.child_frame_id == "odom"
                and tf.header.frame_id == "map"
                and self.T_map_to_odom is None
            ):
                self.T_map_to_odom = self.tfTransform_to_matrix(tf.transform)
                self.get_logger().info(f"Map -> odom transformation acquired.\n")

        if (
            self.Q1 is not None
            and self.Q2 is not None
            and self.Q3 is not None
            and self.T_odom_to_base_link is not None
            and self.T_map_to_odom is not None
        ):
    ...
    def update_calibration(self, position):
        ...
        if (
            self.prism_id == -1
            or self.euclidian_distance(position, self.prism_positions[self.prism_id])
            > 0.3
            and self.nb_poses >= self.min_measurements
        ):
            self.prism_id += 1
            ...
            self.get_logger().info(
                f"Prism {self.prism_id+1}: {self.nb_poses} measurements."
            )
            self.move_announced = False

        elif (
            self.euclidian_distance(position, self.prism_positions[self.prism_id])
            < 0.03
            and self.nb_poses < self.min_measurements
        ):
            self.current_sums += position
        ...
        elif not getattr(self, "move_announced", False):
            self.get_logger().info(
                f"Prism {self.prism_id+1} acquired, move to Prism {self.prism_id+2}.\n"
            )
            self.move_announced = True
    ...
    def publish_pose(self, position):
        ...
        if not getattr(self, "pose_announced", False):
            self.get_logger().info(f"Publishing pose to {self.publisher.topic}.")
            self.pose_announced = True
